...
const Settings = () => {
  ...
  const { updateUserProfile, tog_successMessage, modal_successMessage } =
    useAuth();
  ...
  const getUserFromSession = JSON.parse(sessionStorage.getItem("userInfo"));

  const [firstName, setFirstName] = useState(
    getUserFromSession?.user?.displayName
      ? getUserFromSession.user.displayName.split(" ")[0]
      : "" || getUserFromSession?.user?.providerData[0]?.displayName
      ? getUserFromSession?.user?.providerData[0]?.displayName.split(" ")[0]
      : ""
  );
  const [lastName, setLastName] = useState(
    getUserFromSession?.user?.displayName
      ? getUserFromSession.user.displayName.split(" ")[1]
      : "" || getUserFromSession?.user?.providerData[0]?.displayName
      ? getUserFromSession?.user?.providerData[0]?.displayName.split(" ")[1]
      : ""
  );
  const [email, setEmail] = useState(
    getUserFromSession?.user?.email
      ? getUserFromSession.user.email
      : "" || getUserFromSession?.user?.providerData[0]?.email
      ? getUserFromSession?.user?.providerData[0]?.email
      : ""
  );
  const [phoneNumber, setPhoneNumber] = useState(
    getUserFromSession?.phoneNumber ? getUserFromSession?.phoneNumber : ""
  );
  ...
  const handleUserInfo = (e) => {
    ...
    updateUserProfile(name, photoURL, parseInt(phoneNumber));
  };
  ...
  return (
    <React.Fragment>
                          ...
                          <Col lg={6}>
                            <div className="mb-3">
                              ...
                              <Input
                                ...
                                //         ?.displayName
                                //     ? userInfo?.user?.providerData[0]?.displayName.split(
                                //         " "
                                //       )[0]
                                //     : ""
                                // }
      ...
      {/* success message that profile updated */}
